...
- `generic_worker` (Terra/medium) owns the explicitly assigned `app/src/game/**` and/or `app/src/content/**` paths.
- `frontend_worker` (Terra/medium) owns `app/src/ui/**`.
- `test_worker` (Luna/high) owns assigned test paths after behavior contracts are stable.
